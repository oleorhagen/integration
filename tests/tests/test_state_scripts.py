...
TEST_STATE_SCRIPT_ERROR = [
    (
        "Error in IDLE_ENTER after authorization",
        {
            "FailureScript": ["Idle_Enter_09"],
            "ExpectedStatus": "success",
            "ScriptOrder": [
    ...
    @pytest.mark.usefixtures("standard_setup_one_client_bootstrapped")
    @pytest.mark.parametrize("description,test_set", TEST_SETS)
    def test_state_script_error_code_error_paths():
        """Makes sure that returning error codes from state-scripts does not cause any infinite loops."""
